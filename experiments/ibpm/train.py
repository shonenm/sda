...
from sda.data import DataRegistry, IBPMDataset
...
def train(i: int):
    ...
    run_name = f"vpsde_w{window}_lr{lr:.0e}_bs{bs}_wd{wd:.0e}_seed{i}"
    ...
    manager = RunManager(
        ...
        name=run_name,
        config=CONFIG,
        dataset_name="ibpm_h5_400x200",
        wandb_project="sda-ibpm",
        wandb_group="ibpm_cylinder_vpsde",
        wandb_tags=["ibpm", "cylinder", "vpsde", f"seed{i}", f"lr{lr:.0e}"],
    )
    ...
    with manager.create_run() as run:
        ...
        for epoch in (bar := trange(epochs, ncols=88)):
            ...
            # モデル保存（50エポックごと）
            if (epoch + 1) % 50 == 0:
                run.save_checkpoint(epoch + 1, score_net.state_dict())

        # 最終モデルの保存
        run.save_checkpoint(epochs, score_net.state_dict(), is_final=True)
